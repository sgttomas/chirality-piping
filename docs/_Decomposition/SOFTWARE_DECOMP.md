...
|SOW-030|IN|The architecture shall support public APIs, plugins, and import/export adapters for integration with external tools.|PRD §19.3|API baseline is schema-first command/query/job result envelopes; public transport protocol and external format list remain TBD.|
...
|SOW-046|IN|The product shall support result exports suitable for review, regression comparison, and downstream tooling.|PRD §15.1, §19.3|Structured result export baseline is schema-first JSON result envelopes for review/regression; additional export formats remain TBD.|
...
|DEL-00-04|Persistence and schema versioning architecture|DATA_MODEL_CHANGE|SOW-059|OBJ-013|L|Architecture contract for deterministic project files, schema versions, migrations, and canonicalization.|
...
|Persistence and reproducibility|Versioned JSON-schema-governed persistence; canonical JSON with JCS-compatible hashing for JSON payloads; manifest hashes for non-JSON/binary assets.|Physical project package/container, migration framework/tooling, binary asset packaging.|
...
|SOW-046|IN|The product shall support result exports suitable for review, regression comparison, and downstream tooling.|PRD §15.1, §19.3|PKG-08|DEL-08-04|OBJ-007|FALSE|Structured result export baseline is schema-first JSON result envelopes for review/regression; additional export formats remain TBD.|
...
|SOW-059|IN|The project shall define persistence format, schema-versioning, migration, canonicalization, and deterministic round-trip strategy before project-file implementation proceeds.|Architecture runway amendment v0.3; SOW-050; PRD §27|PKG-00|DEL-00-04|OBJ-013|FALSE|Persistence architecture basis selected by SCA-001; physical container and migration framework remain TBD.|
...
|OI-004|Specific supported import/export formats remain TBD; schema-first result envelopes are the dispatch baseline.|SOW-030,SOW-046,SOW-062|PKG-10 / human product decision|
...
PREPARATION may scaffold package and deliverable folders from this decomposition and the companion registers. `PKG-00` remains `SEMANTIC_READY`, not `ISSUED`, but its SCA-001 architecture basis may be injected into `PKG-01` through `PKG-12` sealed contexts and future TASK briefs. Type 2 execution still requires one sealed deliverable context, explicit write scope, applicable invariants, and acceptance criteria. Dependency DAG/blocker computation remains disabled until a human-approved acyclic DAG exists.
