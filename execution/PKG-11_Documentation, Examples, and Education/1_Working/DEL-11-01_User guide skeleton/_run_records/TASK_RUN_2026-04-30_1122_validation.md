---
run-id: TASK_RUN_2026-04-30_1122_validation
...
| `python3 tools/validation/validate_dependencies_schema.py <deliverable-folder>/Dependencies.csv` | VALID: 29 required columns, 0 extension columns, 15 data rows. |
...
| `rg -n 'INTERSECTION_OR_OPERATOR_LEAK_PATTERN' _SEMANTIC.md` | PASS: no algebra-leak matches; command exited 1 because no matches were found. |
| `rg -c '^\"v3\\.1\"' Dependencies.csv` | PASS: 15 active data rows observed. |
| `tools/validation/validate_id_format.sh DEL DEL-11-01` | WARNING: helper expects legacy `DEL-[0-9]{3}-[0-9]{2}` and rejects current SOFTWARE_DECOMP ID. |
| `tools/validation/validate_id_format.sh PKG PKG-11` | WARNING: helper expects legacy `PKG-[0-9]{3}` and rejects current SOFTWARE_DECOMP ID. |

## Boundary Review

- Search hits for certification, approval, endorsement, sealing, authentication, code compliance, protected standards, private data, proprietary content, and `ISSUED` occur in prohibitions, boundary statements, run records, dependencies, or source context only.
- No artifact was moved to `ISSUED`.
- No repository-level artifact outside the deliverable write scope was edited.
- `docs/user_guide/index.md`, examples, source code, and repo-level docs were not edited.
