# Dependencies: DEL-05-02 Load-case algebra engine

## Coordination (human-owned)
- **Coordination Mode:** NOT_TRACKED
- **Notes:** No human-declared dependency list was provided for PREPARATION.

## Upstream (I need these before I can proceed) - human-owned declarations
- Dependencies coordinated externally by humans.

## Downstream (These need me) - human-owned declarations
- Dependencies coordinated externally by humans.

## Extracted Dependency Register (populated by TASK+dependency-extract)
- **Status:** UPDATED
- **Dependencies.csv:** `Dependencies.csv`
- **Schema:** v3.1
- **Summary:** 7 ACTIVE rows: 3 ANCHOR, 4 EXECUTION.

| DependencyID | Class | Direction | Type | Target | Status |
|---|---|---|---|---|---|
| DEP-05-02-001 | ANCHOR | UPSTREAM | OTHER | SOW-014 | ACTIVE |
| DEP-05-02-002 | ANCHOR | UPSTREAM | OTHER | OBJ-003 | ACTIVE |
| DEP-05-02-003 | ANCHOR | UPSTREAM | OTHER | OBJ-005 | ACTIVE |
| DEP-05-02-004 | EXECUTION | UPSTREAM | CONSTRAINT | Invariant catalog | ACTIVE |
| DEP-05-02-005 | EXECUTION | UPSTREAM | CONSTRAINT | Architecture basis constraints | ACTIVE |
| DEP-05-02-006 | EXECUTION | UPSTREAM | INTERFACE | DEL-05-01 | ACTIVE |
| DEP-05-02-007 | EXECUTION | DOWNSTREAM | INTERFACE | DEL-06-01 | ACTIVE |

## Run Notes and History (populated by TASK+dependency-extract)
- **Run mode:** UPDATE
- **Strictness:** CONSERVATIVE
- **Scope:** DEL-05-02
- **Run root:** `/Users/ryan/ai-env/projects/chirality-piping/execution`
- **Decomposition path:** `/Users/ryan/ai-env/projects/chirality-piping/docs/_Decomposition/SOFTWARE_DECOMP.md`
- **Source docs:** AUTO; scanned deliverable-local four-document kit, `_REFERENCES.md`, existing `_DEPENDENCIES.md`, and decomposition/register references named in the sealed brief.
- **Warnings:** None. Parent anchor is present exactly once for SOW-014. Interface rows are conservative proposals and retain `SatisfactionStatus=PENDING`.
- **2026-04-30:** TASK+dependency-extract updated `Dependencies.csv` v3.1 with conservative anchor and execution edges.

## Lifecycle Summary (populated by TASK+dependency-extract)
- ACTIVE: 7
- RETIRED: 0
- SatisfactionStatus:
  - SATISFIED: 5
  - PENDING: 2

## Consumer Handoff Notes
- No explicit `CONSUMER_CONTEXT` was supplied. Downstream consumers should treat DEL-05-01 and DEL-06-01 rows as conservative interface proposals, not as scheduling commitments.

