...
|SCA-002-DEC-006|Apply hindsight-review cleanup so deliverables only claim package-local scope, scope-ledger mappings are reciprocal, package assignments match the ledger, latest pointer is current, and supersession CSVs use the canonical tool schema.|`REVIEW_HANDOFF_TO_NEXT_INSTANCE.md`; SOFTWARE_DECOMP single-domain deliverable invariant; local validation results.|Applied; downstream use remains pending human acceptance or explicit refresh authorization.|
