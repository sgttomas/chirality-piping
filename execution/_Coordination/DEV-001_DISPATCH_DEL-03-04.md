---
doc_id: DEV-001-DISPATCH-DEL-03-04
...
# DEV-001 Dispatch - DEL-03-04

## Dispatch Decision

The human project authority authorized ORCHESTRATOR to select exactly one next
bounded DAG item after the `DEL-03-03` handoff correction and push.
ORCHESTRATOR selected `DEL-03-04 - Branch connection component model fields`.
...
| Objectives | `OBJ-004` |
| Context envelope | `M` |
| Deliverable path | `execution/PKG-03_Piping Components, Materials, and Library Data Model/1_Working/DEL-03-04_Branch connection component model fields` |
| Current lifecycle | `SEMANTIC_READY` |

## Selection Rationale

`DEL-03-04` is the next bounded PKG-03 component-family slice after the
generic component schema and bend/elbow fields. It adds branch-connection
geometry, reinforcement, user SIF/flexibility, and source metadata slots before
downstream viewport, editor, and solver consumers depend on branch component
records.
...
No `CANDIDATE` edge currently gates `DEL-03-04`.
...
Resolved baseline to preserve: branch connection values are user-supplied or
reviewed governed data. Public schema slots are allowed; protected SIF,
flexibility, reinforcement, code-specific geometry, proprietary catalog values,
and private library data are not public defaults.

Remaining implementation-level TBDs are not resolved by this dispatch:
accepted public branch-connection source catalogs, public fixture value policy,
exact solver use of user branch flexibility inputs, concrete import formats,
...
- `schemas/component.schema.yaml` has explicit branch-connection family
  contract fields for geometry, reinforcement, user SIF/flexibility inputs,
  source metadata, mechanics-interface use, diagnostics, and open decisions.
- Public fixture data remains invented, schema-shape-only, or missing-value
  evidence; it does not contain protected branch tables, code-specific SIF or
  flexibility values, proprietary catalog values, or private library data.
- Tests verify branch component type coverage, geometry slots,
  reinforcement/source slots, user-supplied modifier slots, family contract
  coverage, and protected-content guardrails.
- Documentation points to the branch fields without claiming engineering
