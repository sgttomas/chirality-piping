...
SCHEDULE interprets work items as defined by the active decomposition agent — **PROJECT_DECOMP** or **SOFTWARE_DECOMP** (Packages / Deliverables and stable IDs), and reads dependency evidence as captured by the **`dependency-extract` skill** (dispatched via TASK; deliverable-local `Dependencies.csv`).
